...
工具栏的「✏️ 管理段位」（需管理员密码验证）可以在页面里直接自定义段位（左侧选项卡切换，右侧编辑面板）：改名、填所属 jubeat 版本、设挑战模式、填金/银/铜奖牌线、改主题色、增删段位、上下排序、增删曲目、改曲名/难度、**上传歌曲图片**（点击曲目行左侧的缩略图选择图片，自动压缩到 240px 内保存），修改实时保存到本机；「恢复默认数据」可回到 `data.json` 的版本，自定义数据会随导出存档一起备份。
